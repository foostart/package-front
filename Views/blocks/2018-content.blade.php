 <?php
$url_path = url('/');
?>

<!--LIST OF POSTS-->
<div class="type-2018">

    <!--ITEM-->
    <div class="blog-content">

        <!--TITLE-->
        <a href="#">
            <h2>Summer Holidays Education</h2>
        </a>
        <!--/TITLE-->

        <!--TAG-->
        <div class="blog-tags">
            Filed in: <a href="#">Online Courses</a> /  Tags: <a href="#">Fashion</a>, <a href="#">Learning</a>, <a href="#">webdesign</a>, <a href="#">Course</a>
        </div>
        <!--/TAG-->

        <!--IMAGE-->
        <div class="thumb">
            <a href="#"><img src="<?php echo $url_path ?>/images/blog-img.jpg" alt=""/></a>
        </div>
        <!--/IMAGE-->

        <!--OVERVIEW-->
        <div class="text">
            <p>
                is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using 'Content here, content here', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text.
            </p>
            <a href="#" class="btn-style">Read More</a>
        </div>
        <!--/OVERVIEW-->

        <!--META-->
        <div class="blog-comments">
            <a href="#"><i class="fa fa-user"></i>David</a>
            <i class="fa fa-calendar"></i>06 Dec, 2011
            <a href="#" class="pull-right"><i class="fa fa-comment"></i>35 Comments</a>
        </div>
        <!--/META-->
    </div>
    <!--/ITEM-->

     <!--ITEM-->
    <div class="blog-content">

        <!--TITLE-->
        <a href="#">
            <h2>Summer Holidays Education</h2>
        </a>
        <!--/TITLE-->

        <!--TAG-->
        <div class="blog-tags">
            Filed in: <a href="#">Online Courses</a> /  Tags:
            <a href="#">Fashion</a>,
            <a href="#">Learning</a>,
            <a href="#">webdesign</a>,
            <a href="#">Course</a>
        </div>
        <!--/TAG-->

        <!--IMAGE-->
        <div class="thumb">
            <a href="#"><img src="<?php echo $url_path ?>/images/blog-img.jpg" alt=""/></a>
        </div>
        <!--/IMAGE-->

        <!--OVERVIEW-->
        <div class="text">
            <p>
                is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using 'Content here, content here', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text.
            </p>
            <a href="#" class="btn-style">Read More</a>
        </div>
        <!--/OVERVIEW-->

        <!--META-->
        <div class="blog-comments">
            <a href="#"><i class="fa fa-user"></i>David</a>
            <i class="fa fa-calendar"></i>06 Dec, 2011
            <a href="#" class="pull-right"><i class="fa fa-comment"></i>35 Comments</a>
        </div>
        <!--/META-->
    </div>
    <!--/ITEM-->


    <!--PAGINATION-->
    <div class="pagination">
        <ul>
            <li><a href="#"><i class="fa fa-angle-left"></i></a></li>
            <li><a href="#">1</a></li>
            <li><a href="#">2</a></li>
            <li><a href="#">3</a></li>
            <li><a href="#">4</a></li>
            <li><a href="#">5</a></li>
            <li><a href="#"><i class="fa fa-angle-right"></i></a></li>
        </ul>
    </div>
    <!--/END PAGINATION-->
</div>

